...
async function getPublicKey (cache) {
  ...
  try {
    debug('JWKS URI:', openIdConfig.jwks_uri)
    const result = await request(openIdConfig.jwks_uri).catch(e => { debug('We got an error on the request:', JSON.stringify(e)); throw e })
